...
const CreateItem: React.FC = () => {
  ...
  const [nuevaCaracteristica, setNuevaCaracteristica] = useState<Caracteristica>({ nombre: '', valor: '' });
  ...
  return (
    <div className="container mt-5">
      <div className="card shadow-lg p-4">
        <h1 className="text-center mb-4 text-success">Crear Nueva Planta</h1>
            ...
            <div className="mb-3">
              ...
              <div className="d-flex">
                {imagenesPreview.map((url, index) => (
                  <img key={index} src={url} alt={`Vista previa ${index}`} className="img-thumbnail me-2" style={{ width: '75px', height: '75px' }} />
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mb-3">
            ...
            <div className="d-flex">
              <input
                ...
                type="text"
                placeholder="Nombre de la característica ej. Género"
              ...
              <input
                className="form-control me-2"
                type="text"
                placeholder="Valor de la característica ej. Rosa"
      ...
      <h2 className="text-center mt-5">Lista de Plantas</h2>
      <table className="table table-striped table-bordered mt-3">
        ...
        <tbody>
          {items.length === 0 ? (
            <tr>
              <td colSpan={4} className="text-center">No hay plantas registradas.</td>
            </tr>
          ) : (
            items.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  {item.imagenes.map((url, index) => (
                    <img key={index} src={url} alt={`Imagen ${index}`} className="img-thumbnail" style={{ width: '50px', height: '50px', marginRight: '5px' }} />
                  ))}
                </td>
                <td>
                  <button
